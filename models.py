...
class BertForAIViVN(nn.Module):
   def __init__(self, bert_type):
       super(BertForAIViVN, self).__init__()
       self.num_labels = 8
       self.config = BertConfig.from_pretrained(bert_type, output_hidden_states=True)
       self.bert = BertModel(config=self.config)
       self.qa_outputs = nn.Linear(4*768, self.num_labels)
